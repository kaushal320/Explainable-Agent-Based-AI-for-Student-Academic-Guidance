...
const Dashboard = ({ user, onLogout }) => {
  ...
  const career = currentUser.prediction.career || "Student";
      ...
      <aside className={`sidebar-scholar ${isSidebarOpen ? "open" : ""}`}>
        <div className="flex items-center gap-4 mb-16 px-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-white/40 uppercase font-black mt-2 tracking-[0.3em]">
              Master v2.0
            </p>
          </div>
        </div>
      ...
      <main className="main-content-scholar">
        <header className="flex justify-between items-center bg-white p-7 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/40 mb-12">
          <div className="flex items-center gap-5">
            ...
            <div className="overflow-hidden">
              <h2 className="text-slate-800 font-black text-xl leading-none mb-1.5 truncate">
                Greetings, {currentUser.name || "Scholar"}
              </h2>
              <div className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full"></div>
                <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] truncate">
                  Portal Status: Optimized
                </p>
              </div>
            </div>
          </div>
        ...
        <AnimatePresence mode="wait">
              ...
              <div className="workspace-grid">
                <div className="scholar-card">
                  <div className="flex items-center gap-3 mb-12">
                    ...
                    <span
                      className="font-black text-indigo-600 uppercase tracking-[0.3em]"
                      style={{ fontSize: "10px" }}
                    >
                      Analysis Output
                    </span>
                  ...
                  <h2 className="text-slate-400 font-bold text-[11px] mb-6 uppercase tracking-[0.4em]">
                    Based on your profile, you are a
                  </h2>
                  ...
                  <div className="p-8 bg-slate-50 rounded-3xl border-l-[6px] border-indigo-600 shadow-sm">
                    <p className="text-slate-600 font-medium text-lg leading-relaxed italic">
                      "Your academic record demonstrates exceptional strength in
                      system logistics and architectural scalability. You are
                      uniquely positioned for high-impact roles in core
                ...
                <div className="scholar-card">
                    ...
                    {[
                      ...
                      <div key={metric.label}>
                        <div className="flex justify-between items-center mb-3 px-1">
                          <span className="font-black text-slate-500 text-[10px] uppercase tracking-[0.2em]">
                            {metric.label}
                          </span>
                          <span className="font-black text-indigo-700 text-xs bg-indigo-50 px-3 py-1 rounded-xl">
                            {metric.score}
                          </span>
                        </div>
                        <div
                          className="progress-track-final"
                          style={{
                            height: "12px",
                            background: "#f1f5f9",
                            borderRadius: "12px",
                  ...
                  <div className="mt-12 pt-8 border-t border-slate-100 flex flex-col gap-4">
                    <div className="flex items-center gap-2 mb-1 px-1">
                      ...
                      <span className="font-black text-indigo-900 text-[9px] uppercase tracking-[0.3em]">
                        AI Commands
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      >
                        ...
                        <div className="text-left flex-1 min-w-0">
                          <span className="block font-black text-[11px] uppercase tracking-[0.05em] leading-tight truncate">
                            Consult AI Tutor
                          </span>
                          <span className="text-[8px] text-white/50 font-medium uppercase tracking-widest block truncate">
                            Neural Guidance
                          </span>
                        </div>
                        <ChevronRight
                      ...
                      <motion.button
                        whileHover={{
                          y: -3,
                          boxShadow: "0 12px 24px rgba(6, 78, 59, 0.1)",
                        }}
                        whileTap={{ scale: 0.98 }}
          ...
          {currentPath === "/app/learning" && (
            ...
            >
              <LearningHub user={currentUser} />
            </motion.div>
          )}

          {currentPath === "/app/roadmap" && (
            <motion.div
              key="roadmap"
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -10 }}
              transition={{ duration: 0.3 }}
              className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl overflow-hidden min-h-[700px] w-full p-8"
            >
              <Roadmap prediction={currentUser.prediction} />
            </motion.div>
          )}
        </AnimatePresence>
      </main>
